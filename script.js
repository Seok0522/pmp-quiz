document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayQuestion(index) {
        ...
        for (const [key, value] of Object.entries(question.options)) {
            if (value) { const optionElement = document.createElement('div'); optionElement.dataset.key = key; optionElement.textContent = `${key}: ${value}`; optionElement.addEventListener('click', () => { if (isSubmitted) return; selectedAnswer = key; document.querySelectorAll('#options-container div').forEach(el => el.classList.remove('selected')); optionElement.classList.add('selected'); }); optionsContainer.appendChild(optionElement); }
        }
