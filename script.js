...
// All functions and variables are defined in the global scope initially,
// and event listeners are added after the DOM is fully loaded.
...
async function showNotesView() {
    ...
    notesBtn.onclick = () => { currentQuestionSet = allQuestions; currentQuestionIndex = 0; displayQuestion(currentQuestionIndex); showQuizView(); };
    ...
    notes.forEach(note => {
        ...
        noteEl.innerHTML = `
            <div><strong>문제 ${note.number}:</strong> ${note.question_ko.substring(0, 50)}...</div>
    ...
    try {
        ...
        Papa.parse(csvData, {
            ...
            complete: (results) => {
                ...
                if (currentQuestionSet.length > 0) displayQuestion(0);
...
// Dynamically load Google API scripts using the .onload handler method
